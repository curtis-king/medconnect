                ...
                <div class="p-10">
                    ...
                    <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                        ...
                        <p class="text-blue-700 dark:text-blue-300">
                            ...
                            <span class="font-bold text-green-600 dark:text-green-400">{{ number_format($frai->prix, 2) }} XAF</span>
                        ...
                        @if($frai->detail)
                            <p class="text-sm text-blue-600 dark:text-blue-400 mt-1">{{ $frai->detail }}</p>
                        @endif
                    </div>

                    <form method="POST" action="{{ route('frais.update', $frai) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input id="libelle" type="text" name="libelle" value="{{ old('libelle', $frai->libelle) }}" required
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input id="prix" type="number" name="prix" value="{{ old('prix', $frai->prix) }}" required step="0.01" min="0"
                        ...
                        <div>
                            ...
                            <select id="type" name="type" required
                                ...
                                @foreach($types as $key => $label)
                                    <option value="{{ $key }}" {{ old('type', $frai->type) === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="detail" name="detail" rows="4"
                                ...
                                placeholder="Description détaillée du frais...">{{ old('detail', $frai->detail) }}</textarea>
                     ...
                     <form action="{{ route('frais.destroy', $frai) }}" method="POST"
                                  onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer définitivement ce frais ?')" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
