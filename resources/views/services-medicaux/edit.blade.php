    ...
    <x-slot name="header">
        <div>
            <h2 class="font-bold text-2xl bg-gradient-to-r from-emerald-600 to-cyan-600 bg-clip-text text-transparent">
                Modifier le Service Médical
            </h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Mettez à jour les informations du service</p>
        </div>
    </x-slot>

    <div class="py-12 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        <form method="POST" action="{{ route('services-medicaux.update', $service) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 p-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nom" value="{{ $service->nom }}" required placeholder="Ex: Consultation généraliste" class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-900 transition-all duration-300 @error('nom') border-red-500 @enderror">
                        @error('nom')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" placeholder="Entrez une description détaillée du service..." class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-900 transition-all duration-300 resize-none @error('description') border-red-500 @enderror">{{ $service->description }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="prix" value="{{ $service->prix }}" required placeholder="0" min="0" step="1" class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-900 transition-all duration-300 @error('prix') border-red-500 @enderror">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 p-8">
                ...
                <div class="space-y-4">
                    <label class="flex items-center p-4 rounded-lg border-2 border-transparent border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-900/30 cursor-pointer hover:border-emerald-300 dark:hover:border-emerald-700 transition-all duration-300">
                        <input type="checkbox" name="actif" value="1" {{ $service->actif ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-emerald-500 focus:ring-emerald-500 cursor-pointer">
                        <div class="ml-3">
