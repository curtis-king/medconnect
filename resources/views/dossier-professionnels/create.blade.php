    ...
    <x-slot name="header">
        <div class="text-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Nouveau Dossier Professionnel') }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700">
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4">
                    <h3 class="text-white font-semibold text-lg">Dossier Professionnel</h3>
                    <p class="text-blue-100 text-sm mt-1">Soumettez votre dossier pour devenir professionnel sur la plateforme.</p>
                </div>
                <div class="p-6">
                    <form method="POST" action="{{ ($selfService ?? false) ? route('user.professional.store') : route('dossier-professionnels.store') }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Raison Sociale <span class="text-gray-400">(optionnel)</span></label>
                                ...
                                <div>
                                    ...
                                    <select name="type_structure" required
                                        ...
                                        <option value="individuel" @selected(old('type_structure') === 'individuel')>Individuel (seul)</option>
                                        <option value="clinique" @selected(old('type_structure') === 'clinique')>Clinique</option>
                                        <option value="hopital" @selected(old('type_structure') === 'hopital')>Hôpital</option>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">NIU <span class="text-gray-400">(Numéro d'Identification Unique)</span></label>
                                    <input type="text" name="NIU" value="{{ old('NIU') }}"
                                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                                    @error('NIU')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="forme_juridique" value="{{ old('forme_juridique') }}"
                                           placeholder="SARL, SA, EI..."
                                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                                    @error('forme_juridique')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Documents -->
                        <div>
                            <h4 class="text-base font-semibold text-gray-900 dark:text-gray-100 mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">Documents Justificatifs</h4>
                            <div class="mb-4">
                                ...
                                <p class="mt-1 text-xs text-gray-400">Individuel: photo de profil. Clinique/hôpital/dispensaire/autre: logo.</p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-gray-400">PDF, JPG, PNG - max 5Mo</p>
                        ...
                        <div>
                            <h4 class="text-base font-semibold text-gray-900 dark:text-gray-100 mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">Frais d'Inscription</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tarif d'inscription <span class="text-red-500">*</span></label>
                                    <select name="frais_id" required
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                                        <option value="">-- Sélectionner --</option>
                                        @foreach($fraisInscription as $frais)
                                            <option value="{{ $frais->id }}" @selected(old('frais_id') == $frais->id)>
                                                {{ $frais->libelle }} — {{ number_format($frais->prix, 0, ',', ' ') }} XAF
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                @if(!($selfService ?? false))
                                    <div>
                                        ...
                                        <select name="mode_paiement_inscription"
                                                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                                            <option value="">-- Sélectionner --</option>
                                            <option value="cash" @selected(old('mode_paiement_inscription') === 'cash')>Cash</option>
                                            <option value="mobile_money" @selected(old('mode_paiement_inscription') === 'mobile_money')>Mobile Money</option>
                                            <option value="virement" @selected(old('mode_paiement_inscription') === 'virement')>Virement</option>
                                            <option value="carte" @selected(old('mode_paiement_inscription') === 'carte')>Carte</option>
                                        </select>
                                        @error('mode_paiement_inscription')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                                    </div>
                                @endif
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Référence de Paiement</label>
                                    <input type="text" name="reference_paiement_inscription" value="{{ old('reference_paiement_inscription') }}"
                                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                                    @error('reference_paiement_inscription')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Notes -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes / Observations</label>
                        ...
                        <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <a href="{{ ($selfService ?? false) ? route('dashboard') : route('dossier-professionnels.index') }}"
                            ...
                            <button type="submit"
                                ...
                                {{ ($selfService ?? false) ? 'Continuer vers paiement' : 'Soumettre le Dossier' }}
                            </button>
                        </div>
                    </form>
                </div>
